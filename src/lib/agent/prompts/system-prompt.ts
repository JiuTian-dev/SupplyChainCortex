 ...
 *
 * Located in lib/agent/prompts/ (not app/api/chat/) so that lib modules
 * (agent/fsm.ts, audit/replay-engine.ts) can import it without a
 * lib → app/api reverse dependency.
 *
 * app/api/chat/chat.prompt.ts re-exports this for backward compatibility.
 */
