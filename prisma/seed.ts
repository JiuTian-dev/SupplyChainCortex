...
function buildCost(_sub: string, idx: number) {
...
async function main() {
  ...
  // ── 1. Products (createMany, pre-generate IDs for FK references) ──────────
  ...
  const productModels: Array<{
    ...
    pricing: ReturnType<typeof buildCost>;
  }> = [];
  ...
  for (const cat of CATEGORIES) {
    for (const sub of cat.subs) {
      ...
      for (let i = 0; i < 4 + Math.floor(rng() * 8); i++) {
        const sku = `${cat.cat.substring(0,2)}-${sub.substring(0,2)}${String(productModels.length + 1).padStart(4,'0')}`;
        const name = buildProductName(sub, productModels.length);
        const pricing = buildCost(sub, productModels.length);
        const abc = productModels.length < PRODUCT_COUNT * 0.15 ? 'A' : productModels.length < PRODUCT_COUNT * 0.45 ? 'B' : 'C';
        productModels.push({ id: uuid(), sku, name, category: cat.cat, subCategory: sub, pricing, abcClass: abc, fsnClass: 'F', unitCost: pricing.rawMaterial + pricing.labor, sellingPrice: pricing.sellingPrice, weight: 0.3 + rng() * 6, origin: rng() > 0.3 ? 'CN' : 'VN' });
        if (productModels.length >= PRODUCT_COUNT) break;
      }
    }
    if (productModels.length >= PRODUCT_COUNT) break;
  }
  ...
  await prisma.product.createMany({
    data: productModels.map(p => ({
      id: p.id, sku: p.sku, name: p.name, category: p.category, subCategory: p.subCategory,
      unitCost: p.unitCost, sellingPrice: p.sellingPrice, weight: p.weight,
      origin: p.origin, abcClass: p.abcClass, fsnClass: p.fsnClass,
    })),
  });
  console.log(`✅ ${productModels.length} 产品`);

  // Inventory (createMany with pre-generated IDs)
  const now = new Date();
  const inventoryData = productModels.map((prod, idx) => {
    ...
    if (statusRoll < 0.12) {
      ...
      quantity = Math.round(safetyStock * (0.3 + rng() * 0.5));
    ...
    return {
      id: uuid(), productId: prod.id, sku: prod.sku, productName: prod.name,
  ...
  console.log(`✅ ${inventoryData.length} 库存`);

  // Cost records (createMany)
  const costData = productModels.map((prod, idx) => {
  ...
  console.log(`✅ ${costData.length} 成本记录`);
  ...
  for (const prod of productModels) {
    const isA = prod.abcClass === 'A';
    const isB = prod.abcClass === 'B';
    const baseQty = isA ? 15 + rng() * 30 : isB ? 5 + rng() * 20 : 1 + rng() * 8;
    ...
    for (let d = 0; d < 365; d++) {
      ...
      if (month === 10 || month === 11) qty *= 1.3 + rng() * 0.8;
      if (month === 1 || month === 7) qty *= 0.7 + rng() * 0.4;
      ...
      salesBatch.push({
        id: uuid(), productId: prod.id, sku: prod.sku, productName: prod.name,
        date: new Date(date.getFullYear(), date.getMonth(), date.getDate()), quantity: qty,
  ...
  // ── 3. Shipments (createMany) ─────────────────────────────────────────────
  const statuses = ['in_transit','in_transit','customs','delivered','delayed','exception'];
  const shipmentData: Array<{
    id: string; productId: string; sku: string; productName: string;
    trackingNumber: string; origin: string; destination: string; carrier: string;
    status: string; eta: Date; delayDays: number; riskLevel: string; events: string;
  }> = [];

  for (let i = 0; i < 120; i++) {
    const prod = productModels[Math.floor(rng() * productModels.length)];
    ...
    const events = [
      { eventTime: new Date(Date.now() - (14 + rng() * 20) * 86400000).toISOString(), location: WAREHOUSES[Math.floor(rng() * WAREHOUSES.length)], description: '包裹已揽收', status: 'picked_up' },
    ];
    if (status !== 'delivered') {
    ...
    shipmentData.push({
      ...
      origin: WAREHOUSES[Math.floor(rng() * WAREHOUSES.length)], destination: dest.dest,
      carrier: CARRIERS[Math.floor(rng() * CARRIERS.length)],
      status, eta: new Date(Date.now() + (7 + rng() * 14) * 86400000),
      delayDays, riskLevel,
  ...
  // ── 4. Suppliers (createMany) ─────────────────────────────────────────────
  await prisma.supplier.createMany({
    data: SUPPLIERS.map(s => ({
      id: uuid(), code: s.code, name: s.name, region: s.region, category: s.cat,
      leadTime: s.lt, rating: s.rating, status: 'active',
    })),
  });
  console.log(`✅ ${SUPPLIERS.length} 供应商`);

  // ── 5. Alert Rules (createMany) ───────────────────────────────────────────
  await prisma.alertRule.createMany({
    data: [
      { id: uuid(), ruleId:'low-stock', name:'低库存预警', field:'quantity', operator:'<', threshold:0.5, unit:'安全库存倍数', enabled:true, severity:'critical' },
      { id: uuid(), ruleId:'overstock', name:'库存积压预警', field:'turnoverDays', operator:'>', threshold:120, unit:'天', enabled:true, severity:'warning' },
      { id: uuid(), ruleId:'slow-moving', name:'滞销预警', field:'turnoverDays', operator:'>', threshold:90, unit:'天', enabled:true, severity:'warning' },
      { id: uuid(), ruleId:'low-margin', name:'低毛利预警', field:'grossMargin', operator:'<', threshold:40, unit:'%', enabled:true, severity:'critical' },
    ],
  });
  console.log('✅ 预警规则');

  // ── 6. Return Records (createMany) ────────────────────────────────────────
  ...
  for (let i = 0; i < 80; i++) {
    ...
    returnData.push({
      id: uuid(), sku: prod.sku, productName: prod.name, quantity: qty, reason,
  ...
  await prisma.returnRecord.createMany({ data: returnData });
  console.log(`✅ ${returnData.length} 退货记录`);

  // ── 7. Defect Records (createMany) ────────────────────────────────────────
  const defectTypes = ['功能','外观','安全','包装'];
  const severities = ['minor','minor','minor','major','major','critical'];
  const defectData: Array<{
    id: string; sku: string; productName: string; defectType: string;
    severity: string; quantity: number; detectedAt: Date;
    rootCause: string; correctiveAction: string; status: string;
  }> = [];
  for (let i = 0; i < 60; i++) {
    const prod = productModels[Math.floor(rng() * productModels.length)];
    defectData.push({
      id: uuid(), sku: prod.sku, productName: prod.name,
  ...
  await prisma.warrantyCost.createMany({ data: warrantyData });
  console.log(`✅ ${warrantyData.length} 质保记录`);

  // ── 9. Compliance Certs (createMany) ──────────────────────────────────────
  ...
  await prisma.complianceCert.createMany({
    data: certs.map((c, i) => ({
      id: uuid(), certName: c.certName, certNumber: `${c.certName}-${past(100+i*20)}`,
  ...
  // ── 10. Regulation Changes (createMany) ───────────────────────────────────
  const regulations = [
  ...
  await prisma.regulationChange.createMany({
    data: regulations.map(r => ({
      id: uuid(), title: r.title, source: r.source, category: r.cat,
      description: `${r.source} ${r.cat} 法规更新`,
      impactLevel: r.impact, effectiveDate: r.eff, deadline: r.dead,
      affectedSkus: '[]', affectedCerts: '[]',
      actionRequired: `按${r.source}新规执行`,
      status: ['new','reviewing','action_required'][Math.floor(rng()*3)],
    })),
  });
  console.log(`✅ ${regulations.length} 法规变更`);

  // ── 11. Supply Chain Events (createMany) ──────────────────────────────────
  const eventsData = [
  ...
  await prisma.supplyChainEvent.createMany({
    data: eventsData.map(e => ({ id: uuid(), ...e })),
  });
  console.log(`✅ ${eventsData.length} 供应链事件`);

  // ── 12. Users ──────────────────────────────────────────────────────────────
  const bcrypt = await import('bcryptjs');
  const [hash, hash2, hash3] = await Promise.all([
  ...
  await prisma.user.createMany({
    data: [
      { id: uuid(), email:'admin@supply-chain.com', name:'管理员', password:hash, role:'admin' },
      { id: uuid(), email:'manager@supply-chain.com', name:'经理', password:hash2, role:'manager' },
      { id: uuid(), email:'viewer@supply-chain.com', name:'查看者', password:hash3, role:'viewer' },
    ],
  });
